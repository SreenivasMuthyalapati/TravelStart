...
public class TravellerDetailsPageMethods {
    ...
    public boolean isFareIncreased(WebDriver driver){
    ...
    public double selectWhatsapp(WebDriver driver, boolean addWhatsapp){
        ...
        if (addWhatsapp){
            ...
            try {
                ...
                whatsAppPrice = m.stringToInteger(whatsAppPriceInSting);
    ...
    public double getBaggageCost(WebDriver driver, String baggageType){
        ...
        if (baggageType.equalsIgnoreCase("Checked")){
            ...
            BaggageCost = m.stringToInteger(costString);
            ...
            List<Integer> baggageCostInInt = new ArrayList<>(List.of());

            for (int i = 0; i< baggageCostElements.size(); i++){

                baggageCostStrings.add(baggageCostElements.get(i).getText());

                baggageCostInInt.add(m.stringToInteger(baggageCostStrings.get(i)));

                BaggageCost = BaggageCost+baggageCostInInt.get(i);
            }
    ...
    public double selectMeal(WebDriver driver, String adultCount, String youngAdultCount,String childCount, String infantCount) throws InterruptedException {
        ...
        for (int i =1; i<= totalPaxCount; i++) {
            ...
            try {
                ...
                Thread.sleep(500);
                double mealCostDouble = m.stringToInteger(mealCost);
    ...
    public boolean isAddOnsOffered(WebDriver driver){

        boolean isAddOnsOffered = false;
        WebElement AddOnsBlock = null;

        try{

            AddOnsBlock = driver.findElement(FlightPage.addOnsCollapsedCard);
        ...
        try {
            ...
            if (AddOnsBlock.isDisplayed()){

                isAddOnsOffered = true;

            }else {

                isAddOnsOffered = false;
            }
        ...
        return isAddOnsOffered;

    }

    public void contiueToNextStep(WebDriver driver){
